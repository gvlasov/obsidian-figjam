	...
	async onload() {
		...
		this.addCommand({
			...
			callback: () => {
				...
				new Notice("This feature is coming soon! For now, use 'Create new FigJam board' to save and open.");
			}
		});
	...
	private createFigJamFile(targetFolder: TFolder | null): void {
		new CreateFigJamModal(this.app, targetFolder, async (data: FigJamFileData, fileName: string) => {
			try {
				...
				new Notice(`Created FigJam board: ${fileName}`);
			} catch (error) {
